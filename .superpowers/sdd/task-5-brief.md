...
**Interfaces:**
- Consumes: `SpreadsheetEngine.getFunctionList()` (from Task 4), current `editValue` from store
- Produces: A popup that appears when user types `=` in a cell, showing filtered function suggestions with keyboard navigation

- [ ] **Step 1: Create FormulaAutocomplete component**

Create `src/components/FormulaAutocomplete.tsx` with the following content:
