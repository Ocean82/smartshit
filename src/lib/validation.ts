  ...
  switch (criteria) {
    case "between":
      if (v.min != null && num < v.min)
        return { valid: false, message: v.message || `Must be ≥ ${v.min}` };
      if (v.max != null && num > v.max)
        return { valid: false, message: v.message || `Must be ≤ ${v.max}` };
      return { valid: true };
    case "notBetween":
      if (v.min != null && v.max != null && num >= v.min && num <= v.max)
        return {
          valid: false,
          message: v.message || `Must not be between ${v.min} and ${v.max}`,
        };
      return { valid: true };
    case "greaterThan":
      if (v.min != null && num <= v.min)
    ...
    case "equalTo":
      if (v.min != null && num !== v.min)
        return { valid: false, message: v.message || `Must equal ${v.min}` };
      return { valid: true };
    case "notEqualTo":
      if (v.min != null && num === v.min)
        return {
          valid: false,
          message: v.message || `Must not equal ${v.min}`,
        };
      return { valid: true };
    default:
...
function validateList(strVal: string, v: DataValidation): ValidationResult {
  ...
  if (v.values && !v.values.includes(strVal)) {
    return {
      valid: false,
      message: v.message || `Must be one of: ${v.values.join(", ")}`,
    };
  }
  return { valid: true };
}
  ...
  switch (criteria) {
    case "length":
      if (v.min != null && strVal.length < v.min)
        return {
          valid: false,
          message: v.message || `Must be at least ${v.min} characters`,
        };
    ...
    case "contains":
      if (
        v.containsText &&
        !strVal.toLowerCase().includes(v.containsText.toLowerCase())
      )
        return {
          valid: false,
          message: v.message || `Must contain "${v.containsText}"`,
        };
      return { valid: true };
    case "notContains":
      if (
        ...
        return {
          valid: false,
          message: v.message || `Must not contain "${v.containsText}"`,
        };
      return { valid: true };
    case "startsWith":
      if (
        v.containsText &&
        !strVal.toLowerCase().startsWith(v.containsText.toLowerCase())
      )
        return {
          valid: false,
          message: v.message || `Must start with "${v.containsText}"`,
        };
      return { valid: true };
    case "endsWith":
      if (
        v.containsText &&
        !strVal.toLowerCase().endsWith(v.containsText.toLowerCase())
      )
        return {
          valid: false,
          message: v.message || `Must end with "${v.containsText}"`,
        };
      return { valid: true };
    default:
      return { valid: true };
    ...
    case "after":
      if (v.dateMin) {
        ...
        if (!isNaN(minDate) && dateVal <= minDate)
          return {
            valid: false,
            message: v.message || `Must be after ${v.dateMin}`,
          };
      }
      return { valid: true };
    case "before":
      if (v.dateMax) {
        ...
        if (!isNaN(maxDate) && dateVal >= maxDate)
          return {
            valid: false,
            message: v.message || `Must be before ${v.dateMax}`,
          };
      }
      return { valid: true };
    case "between":
      if (v.dateMin) {
        ...
        if (!isNaN(minDate) && dateVal < minDate)
          return {
            valid: false,
            message: v.message || `Must be on or after ${v.dateMin}`,
          };
      }
      if (v.dateMax) {
        const maxDate = Date.parse(v.dateMax);
        if (!isNaN(maxDate) && dateVal > maxDate)
          return {
            valid: false,
            message: v.message || `Must be on or before ${v.dateMax}`,
          };
      }
      return { valid: true };
    default:
...
function validateCheckbox(strVal: string, v: DataValidation): ValidationResult {
  ...
  // Allowlist-sanitise checkedValue/uncheckedValue so no tainted data flows
  // into comparisons or error messages (eliminates false-positive CWE-94/78).
  const CHECKBOX_ALLOWLIST: readonly string[] = [
    "TRUE", "FALSE", "YES", "NO", "1", "0", "ON", "OFF",
  ];
  const sanitise = (raw: string | undefined, fallback: string): string => {
    const upper = String(raw ?? "").toUpperCase();
    return CHECKBOX_ALLOWLIST.includes(upper) ? upper : fallback;
  };
  const checked = sanitise(v.checkedValue, "TRUE");
  const unchecked = sanitise(v.uncheckedValue, "FALSE");
  const upper = String(strVal).toUpperCase();

  if (upper === checked || upper === unchecked) {
    return { valid: true };
  }

  if (CHECKBOX_ALLOWLIST.includes(upper)) {
    return { valid: true };
  }

  return {
    valid: false,
    message: v.message || `Must be "${checked}" or "${unchecked}"`,
  };
}
...
function validateFormula(
  ...
  return result
    ...
    : {
        valid: false,
        message: v.message || `Validation rule failed: ${expression}`,
      };
}
...
function evaluateSafeExpression(
  ...
  const betweenMatch =
    /^value\s*(>=?)\s*(-?\d+(?:\.\d+)?)\s*&&\s*value\s*(<=?)\s*(-?\d+(?:\.\d+)?)$/.exec(
      expr,
    );
  if (betweenMatch) {
    if (isNaN(numVal)) return false;
    const left = compareOp(
      numVal,
      betweenMatch[1],
      parseFloat(betweenMatch[2]),
    );
    const right = compareOp(
      numVal,
      betweenMatch[3],
      parseFloat(betweenMatch[4]),
    );
    return left && right;
  }
  ...
  // expr is matched against a strict allowlist regex — no code is executed (false-positive CWE-94/78)
  const lengthMatch = /^value\.length\s*(>|>=|<|<=|===?|!==?)\s*(\d+)$/.exec(
    expr,
  );
  if (lengthMatch) {
