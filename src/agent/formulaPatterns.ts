...
export function parseAdvancedFormula(message: string): ParsedFormulaRequest | null {
  ...
  const countIf = message.match(new RegExp(
    `\\b(?:add|create|insert|write|put)\\s+(?:a\\s+)?COUNTIF(?:\\s+formula)?\\s+(?:in|at|into)\\s+(${CELL_REF})\\s+(?:to\\s+)?count\\s+(?:cells?\\s+)?(?:in\\s+)?(${RANGE_REF})\\s+(?:that\\s+are\\s+)?(equal(?:s)?(?:\\s+to)?|is|=|>=|<=|>|<|over|above|under|below|at\\s+least|at\\s+most)\\s+(.+?)\\s*$`,
    'i',
  ))
  if (countIf) {
  ...
  const sumIf = message.match(new RegExp(
    `\\b(?:add|create|insert|write|put)\\s+(?:a\\s+)?SUMIF(?:\\s+formula)?\\s+(?:in|at|into)\\s+(${CELL_REF})\\s+(?:to\\s+)?sum\\s+(${RANGE_REF})\\s+where\\s+(${RANGE_REF})\\s+(equal(?:s)?(?:\\s+to)?|is|=|>=|<=|>|<|over|above|under|below|at\\s+least|at\\s+most)\\s+(.+?)\\s*$`,
    'i',
  ))
  if (sumIf) {
  ...
  const ifFormula = message.match(new RegExp(
    `\\b(?:add|create|insert|write|put)\\s+(?:an?\\s+)?IF(?:\\s+formula)?\\s+(?:in|at|into)\\s+(${CELL_REF})\\s*:?\\s*if\\s+(${CELL_REF})\\s+(equal(?:s)?(?:\\s+to)?|is(?:\\s+not)?|not\\s+equal\\s+to|==|=|!=|<>|>=|<=|>|<|over|above|under|below|at\\s+least|at\\s+most|greater\\s+than|less\\s+than)\\s+(.+?)\\s+then\\s+(.+?)\\s+(?:else|otherwise)\\s+(.+?)\\s*$`,
    'i',
  ))
  if (ifFormula) {
